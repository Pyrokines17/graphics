...
//        });
        ...
        addMouseWheelListener(e -> {
            nearPlane += e.getWheelRotation() * 1;
            repaint();
        });
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (drawRays) {
            ...
            List<List<List<Vector>>> spheresPoints = model.getSpheresPoints();
            List<List<List<Vector>>> newSpheresPoints = applyTransformationsSpheres(spheresPoints);

            g2d.setColor(Color.BLACK);
            drawSpheres(g2d, newSpheresPoints);
        }
    }

    private void drawSpheres(Graphics2D g2d, List<List<List<Vector>>> spheresPoints) {
        for (List<List<Vector>> sphere : spheresPoints) {
            for (List<Vector> circle : sphere) {
                GeneralPath path = new GeneralPath();
                for (int i = 0; i < circle.size(); i++) {
                    Vector point = circle.get(i);
                    if (i == 0) {
                        path.moveTo(centralizeX(point.getX()), centralizeY(point.getY()));
                    } else {
                        path.lineTo(centralizeX(point.getX()), centralizeY(point.getY()));
                    }
                }
                path.closePath();
                g2d.draw(path);
...
//                matrix);
    ...
    private List<List<List<Vector>>> applyTransformationsSpheres(List<List<List<Vector>>> spheresPoints) {
        List<List<List<Vector>>> transformedSpheres = new ArrayList<>();

        double focalLength = 1;
        double theta = Math.toRadians(90);
        double h = Math.tan(theta / 2.0);
        double viewportHeight = 2.0 * h * focalLength;
        double viewportWidth = viewportHeight * ((double) 640 / 400);

        double[][] matrix = Matrix.getScalingMatrix(1.0, 1.0, 1.0);
        matrix = Matrix.multiply(
                Matrix.getLookAt(
                        new Vector(0, 0, 0),
                        new Vector(0, 0, -1),
        ...
        for (List<List<Vector>> sphere : spheresPoints) {
            List<List<Vector>> transformedSphere = new ArrayList<>();
            for (List<Vector> circle : sphere) {
                List<Vector> transformedCircle = new ArrayList<>();
                for (Vector point : circle) {
                    Vector transformedPoint = Matrix.multiply(matrix, point);
                    transformedCircle.add(new Vector(
                            transformedPoint.getX() / transformedPoint.getW(),
                            transformedPoint.getY() / transformedPoint.getW(),
                            transformedPoint.getZ() / transformedPoint.getW()));
                }
                transformedSphere.add(transformedCircle);
            }
            transformedSpheres.add(transformedSphere);
        }
